...
**STEP 4: Check for THRESHOLD**
If the query asks which locations exceed or fall below a numeric value for some portion of time:
- Keywords: "above", "below", "over", "under", "exceed", "less than", "greater than", "more than", "at least", "no more than"
- Examples:
  * "Which nodes had temperature above 25 degrees?" → threshold
  * "Nodes where humidity exceeded 80% last week" → threshold
...
- Set: threshold_operator = ">", ">=", "<", or "<=" based on the query wording
  * "above", "over", "exceed", "greater than", "more than" → ">"
  * "at least", "no less than" → ">="
  * "below", "under", "less than" → "<"
  * "no more than", "at most" → "<="
- Set: aggregation_level = null
- NOTE: threshold differs from comparison — it scans ALL locations, not a named pair

**STEP 5: DEFAULT to QUERY**
...
CRITICAL VALIDATION:
- threshold → location MUST be a list of all available locations, threshold_value MUST be numeric, threshold_operator MUST be one of ">"|">="|"<"|"<=", aggregation_level = null
...
3. "above" OR "below" OR "exceed" OR "over" OR "under" (with a number) → threshold
4. "compare" OR "vs" → comparison
5. "average" OR "max" OR "min" → query with that operation
